 ...
 */
export default async function resolveIpldPath(helia, sourceCid, path) {
  let nodes = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
  let pathBoundaries = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [];
  const {
    value,
    remainderPath
  } = await ipldGetNodeAndRemainder(helia, sourceCid, path);
  if (sourceCid == null) {
